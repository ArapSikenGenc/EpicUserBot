...
async def logo_gen(event):
    ...
    if not ad:
        await event.edit("`Logo hazırlayabilmem için bir isime ihtiyacım var !`")
    bg_, font_ = "", ""
    ...
    if os.path.exists(fayladi):
        await event.client.send_file(
            event.chat_id,
            file=fayladi,
            caption=f"[EpicUserBot](https://t.me/epicuserbot) **{DEFAULT_NAME}** için logo hazırladı.",
            force_document=True,
        )
        os.remove(fayladi)
        await event.delete()
...
Help = CmdHelp('logo')
Help.add_command('logo', '<yazı>', '@EpicUserBot sizin için logo hazırlar.')
Help.add_info('@EpicUserBot - @ByMisakiMey ')
Help.add() 