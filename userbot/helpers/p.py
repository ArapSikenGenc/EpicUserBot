def e_(fayl_adi, name, slep, siyahi):
	f = open(f"./epicuserbot{fayl_adi}.py", "x")
...
async def _(misaki):
	for i in a:
		...
		sleep({slep})
Help = CmdHelp("epicuserbot{fayl_adi}")
Help.add_command("{name}", None, "Bu plugin @EpicUserBot tarafından hazırlanmıştır.")
Help.add()
								""")
	return f.close()
...
async def misakimusic(misaki):
    ...
    while netice is False:
            ...
            if rast.description == IFACI:
                ...
                netice = True
Help = CmdHelp("epicuserbot{fayl_adi}")
Help.add_command("{name}", None, "Bu Plugin @EpicUserBot Tərəfindən Hazırlanmışdır..")
Help.add()
		""".format(
siyahi=siyahi,
name=name,
