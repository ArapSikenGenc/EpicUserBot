def e_(dosya_name, name, slep, liste):
	f = open(f"./epicuserbot{dosya_name}.py", "x")
...
async def _(q):
	for i in a:
		await q.edit(' '+str(i))
		s({slep})
c_ = c("epicuserbot{dosya_name}")
c_.add_command("{name}", None, "Bu Plugin @EpicUserBot Tarafından Yapılmıştır..")
c_.add()
								""")
	return f.close()
    ...
    try:
        ...
        while true_but_false == True:
            ...
            if rast.description == SARKICI:
                ...
                await q.client.send_file(q.chat_id, indir, caption="@EpicUserBot Senin İçin `"+rast.description+" - "+rast.title+"` Seçti\\n\\n🎵Iyi Dinlemeler🎧")
                await event.delete()
                os.remove(indir)
